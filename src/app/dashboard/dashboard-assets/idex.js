...
$(document).ready(function() {
	...
	(function ( $ ) {
		...
		if (jQuery.isFunction(jQuery.fn.typeahead)) {
			// Typeahead
			$.fn.idex_typeahead = function(options) {
				var settings = $.extend({
					hint: false,
					highlight: true,
					minLength: 1,
			...
			$.fn.idex_typeahead_complex = function(options) {
				var settings = $.extend({
					hint: false,
					highlight: true,
					minLength: 1,
		...
		if (jQuery.isFunction(jQuery.fn.autocomplete)) {
			...
			$.fn.idex_autocomplete = function(options) {
				var settings = $.extend({
					appendTo: null,
					source: "",
					minLength: 1,
			...
			$.fn.idex_multicomplete = function(options) {
				var settings = $.extend({
					appendTo: null,
					source: "",
					minLength: 1,
	...
	$('#Update:not(.onEventAttached)').addClass('onEventAttached').on('click', function(e) {
		e.preventDefault();
		$('form#MainForm').submit();
	});

	$('.FormUpdate:not(.onEventAttached)').addClass('onEventAttached').on('click', function(e) {
		e.preventDefault();
		$('form#MainForm').submit();
	});
	...
	// To be removed. Old jQuery UI Datepicker
	...
	if (jQuery.isFunction(jQuery.fn.datetimepicker)) {
		...
		$('.DateTimePickerNoFuture').datetimepicker({
			format: 'DD-MM-YYYY HH:mm:ss',
			useCurrent: false,
			maxDate: moment().format('DD-MM-YYYY HH:mm:ss'),
			calendarWeeks: true,
			showTodayButton: true,
			allowInputToggle: true,
			showClose: true,
	...
	function cloneRow(template_row) {
		...
		$(template_row).find('label').each(function(index, el) {
			$(template_row).find('label').eq(index).html('');
		});
...
function datetimeRangePicker(options) {
	...
	var settings = $.extend({
		...
		from_date: (month > 2 ? moment().date(1).month(3).hour(0).minute(0).seconds(0).format('YYYY-MM-DD HH:mm:ss') : moment().date(1).month(3).year(moment().year()-1).format('YYYY-MM-DD HH:mm:ss')),
		to_date:   (month > 2 ? moment().date(31).month(2).year(moment().year()+1).hour(0).minute(0).seconds(0).format('YYYY-MM-DD HH:mm:ss') : moment().date(31).month(2).format('YYYY-MM-DD HH:mm:ss')),
		ranges: { }
	}, options);
	
	if (Object.keys(settings.ranges).length == 0) {
		settings.ranges = {
			'Today':                 [moment().hour(0).minute(0).seconds(0), moment()],
			'Yesterday':             [moment().hour(0).minute(0).seconds(0).subtract(1, 'days'), moment().hour(23).minute(59).seconds(59).subtract(1, 'days')],
			'This Week':             [moment().hour(0).minute(0).seconds(0).startOf('week'), moment().endOf('week')],
			'This Month':            [moment().hour(0).minute(0).seconds(0).startOf('month'), moment().endOf('month')],
			'Last Month':            [moment().hour(0).minute(0).seconds(0).subtract(1, 'months').startOf('month'), moment().subtract(1, 'months').endOf('month')],
	...
	if (jQuery.isFunction(jQuery.fn.daterangepicker)) {
		$('#ReportRange').daterangepicker({
			...
			minDate: moment((settings.years[0]-1) + '-04-01 00:00:00', 'YYYY-MM-DD HH:mm:ss'),
